<script lang="ts">
  const spots = Array.from({ length: 200 }, () => ({
    top: Math.random() * 100,
      ...
      >
        <defs>
          <linearGradient id="grad{i}" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" style="stop-color:{'#'+(Math.random() * 0xFFFFFF << 0).toString(16).padStart(6, '0')};stop-opacity:1" />
            <stop offset="100%" style="stop-color:{'#'+(Math.random() * 0xFFFFFF << 0).toString(16).padStart(6, '0')};stop-opacity:1" />
          </linearGradient>
        </defs>
        <path
